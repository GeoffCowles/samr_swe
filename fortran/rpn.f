...
c ixy - direction (ixy = 1 for x, ixy=2 for y)
c maxm - max(mx,my)
c meqn - 3
c mwaves -3
c mbc - number of ghost cell layers = ?
c mx - #internal cells in i-dir
c 
c 
...
cgwc      include "gparms.i"
cgwc      common /cmcapa/  mcapa
cgwc      common /comxyt/ dtcom,dxcom,dycom,tcom,icom,jcom
cgwc  set global parameters (should be done through modules)
cgwc      g=grav
      g = 9.8016
cgwc  mindepth
cgwc      abs_tol = 1e-5
...
CGWC        if (mcapa.gt.0) then
C          do i=2-mbc,mx+mbc
C           if (ixy.eq.1) then
C             dxdc=(Rearth*pi/180.d0)
C           else
C 	    dxdc=auxl(i,3)
C           endif
C 
...
C c               # shouldn't happen unless h > 10 km!
C                 write(6,*) 'speed > 316: i,mw,s(i,mw): ',i,mw,s(i,mw)
C                 endif
C 	     s(i,mw)=dxdc*s(i,mw)
C              do m=1,meqn
C                fwave(i,m,mw)=dxdc*fwave(i,m,mw)
C              enddo
C           enddo
C          enddo
CGWC         endif
...
cgwc  binding for f90 f77 
      double precision grav_f77
      common /f90_2f77/  grav_f77
